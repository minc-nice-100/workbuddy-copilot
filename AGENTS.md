...
> 上层 `~/projects/AGENTS.md`（RIPER-5）与全局规则仍生效；本文件为项目特定约定。
...
- 实施计划：[.Codex/memory-bank/plans/target-arch-rebuild.md](.Codex/memory-bank/plans/target-arch-rebuild.md)（STATUS: APPROVED，Phase 0-4）
...
- 编码方式：**codex 子代理**（codex CLI + tcd 可用，支持 worktree 并行 Mode D）。Codex 主 Agent **不写业务代码**，只负责准备/部署/P0-P1 验证/监控/集成。
